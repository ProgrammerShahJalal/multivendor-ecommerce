...
const CheckoutForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (intentError) {
            ...
            await fetch('https://young-springs-82149.herokuapp.com/dashboard/orders', {
                method: 'POST',
                headers: {
                    'content-type': 'application/json'
                },
    ...
    return (
        <div className="h-screen grid grid-cols-3">
            <div className="lg:col-span-2 col-span-3 bg-indigo-50 dark:bg-slate-800 space-y-8 px-12">
                <div className="mt-8 p-4 relative flex flex-col sm:flex-row sm:items-center bg-white dark:bg-slate-600 shadow rounded-md ">
                    ...
                    <div className="text-sm tracking-wide text-gray-500 dark:text-white  mt-4 sm:mt-0 sm:ml-4">Complete your shipping and payment details below.</div>
                ...
                <div className="rounded-md">
                    <form onSubmit={handleSubmit} className='p-3' id="payment-form" method="POST" action="">
                        <div>
                            <h2 className="uppercase tracking-wide text-lg font-semibold text-gray-700 dark:text-white my-2">Shipping & Billing Information</h2>
                            <fieldset className="mb-3 bg-white dark:bg-slate-600 shadow rounded text-gray-600 dark:text-white">
                                <label className="flex border-b border-gray-200 h-12 py-3 items-center">
                                    <span className="text-right px-2">Name</span>
                                    <input name="name" ref={nameRef} className="focus:outline-none px-3 dark:bg-slate-600" placeholder="Try Odinsson" />
                                </label>
                                <label className="inline-flex w-2/4 border-b border-gray-200 py-3">
                                    <span className="text-right px-2">Email</span>
                                    <input defaultValue={user.email} name="email" ref={emailRef} className="focus:outline-none px-3 dark:bg-slate-600" placeholder="unitymart@gmail.com" />
                                </label>
                                <label className="xl:w-2/4 xl:inline-flex py-3 items-start  border-b border-gray-200">
                                    ...
                                    <input name="phone" ref={phoneRef} className="focus:outline-none px-3 dark:bg-slate-600" placeholder="+880170000" />
                                </label>
                                <label className="flex border-b border-gray-200 h-12 py-3 items-center">
                                    <span className="text-right px-2">Address</span>
                                    <input name="address" ref={addressRef} className="focus:outline-none px-3 dark:bg-slate-600" placeholder="10 Street XYZ 654" />
                                </label>
                                <label className="flex border-b border-gray-200 h-12 py-3 items-start">
                                    <span className="text-right px-2">City</span>
                                    <input name="city" ref={cityRef} className="focus:outline-none px-3 dark:bg-slate-600" placeholder="San Francisco" />
                                </label>
                                <label className="inline-flex w-2/4 border-gray-200 py-3">
                                    <span className="text-right px-2 ">State</span>
                                    <input name="state" ref={stateRef} className="focus:outline-none px-3 dark:bg-slate-600" placeholder="CA" />
                                </label>
                                <label className="xl:w-1/4 xl:inline-flex py-3 items-start flex xl:border-none border-t border-gray-200">
                                    ...
                                    <input name="postal_code" ref={zipRef} className="focus:outline-none px-3 dark:bg-slate-600" placeholder="98603" />
                                </label>
            ...
            <div className="col-span-1 bg-white dark:bg-slate-800 dark:text-white lg:block hidden">
                <h1 className="py-6 border-b-2 text-xl text-gray-600 dark:text-white px-8">Order Summary</h1>
                        ...
                        cart.map(item => {
                            return <li className="grid grid-cols-6 gap-2 border-b-1">
                                ...
                                <div className="flex flex-col col-span-3 pt-2">
                                    <span className="text-gray-600 dark:text-white text-md font-semi-bold">{item.title}</span>
                                    <span className="text-gray-400 dark:text-white text-sm inline-block pt-2">{item.category}</span>
                                </div>
                                <div className="col-span-2 pt-3">
                                    <div className="flex items-center space-x-2 text-sm justify-between">
                                        <span className="text-gray-400 dark:text-white">{item.cartQuantity} x ${item.price}</span>
                ...
                <div className="px-8 border-b">
                    <div className="flex justify-between py-4 text-gray-600 dark:text-white">
                        <span>Subtotal</span>
                ...
                <div className="font-semibold text-xl px-8 flex justify-between py-8 text-gray-600 dark:text-white">
                    <span>Total</span>
                    <span>${cartTotal}</span>
                </div>
            </div>
