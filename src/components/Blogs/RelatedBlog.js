...
const RelatedBlog = ({ blog }) => {
    ...
    return (
        <div>
            <Link to={`/blogs/details/${_id}`}>
                <div className="bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-xl shadow-md overflow-hidden">
 ...
 block mt-1 leading-tight font-medium text-gray-800 dark:text-white dark:text-white hover:underline truncate w-36">{title}</Link> <p>{date}</p>
                            <p>{author}</p>
                        </div>
                    </div>
                </div>
