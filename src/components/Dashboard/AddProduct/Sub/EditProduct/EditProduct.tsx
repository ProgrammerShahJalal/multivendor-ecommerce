...
const EditProduct: React.FunctionComponent = () => {
    ...
    useEffect(() => {
        ...
        if (productData.length === 0) {
            fetch(`https://unitymart-admin.onrender.com/product/${id}`)
                .then(res => res.json())
                .then(data => {
    ...
    const handleProductSubmit = async (e: React.FormEvent<HTMLFormElement>, id) => {
        ...
        setIsLoading(true)
        fetch(`https://unitymart-admin.onrender.com/dashboard/updateProduct/${id}`, {
            method: 'PUT',
            headers: {
    ...
    //     // fetch('https://unitymart-admin.onrender.com/addProduct', {
    //     //     method: 'post',
    //     //     body: formData
    //     // })
    //     //     .then(response => response.json())
