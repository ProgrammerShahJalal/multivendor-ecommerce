...
export default function AffiliateLinks() {
    ...
    const deleteUrl = (shortUrl) => {
        fetch(`https://guarded-ocean-73313.herokuapp.com/delete/shortUrl/${shortUrl}`, {
            method: 'DELETE'
        }).then(res => res.json())
            .then(data => {
                console.log(data);
                                            ...
                                            loadings ? 'Loading' :
                                                !links?.length ? 'No URLs Found' : links?.map(link => {
                                                    return <tr>
                                                        ...
                                                        <td className="p-2 whitespace-nowrap">
                                                            <div className="text-left"><a href={`https://guarded-ocean-73313.herokuapp.com/${link.short}`}>{link.short}</a> <i className="fa-solid fa-copy cursor-pointer"
                                                                onClick={() => copyText(`https://guarded-ocean-73313.herokuapp.com/${link.short}`)} title="Copy"></i></div>
                                                        </td>
                                                        <td className="p-2 whitespace-nowrap">
